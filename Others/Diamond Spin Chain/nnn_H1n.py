'''Keep in mind that this directory explores the next-nearest neighbour
Heisenberg interaction in a spin-half chain.

We have divided the energy operator in a spin-half chain, 
between the nth electron site and the (n+2)th site into three parts: 
    
    Hn = H1n + H2n + H3n

This function seeks to apply the first part of the energy operator between the
nth site and the (n+2)th site, which is (Snz S(n+2)z) to the horizontal eigenstates,
...
def nnn_H1n(n, j2, l):
    ...
    for i in range(dim): #All 4 possible cases are taken into account below.
        ...
        if list1[i][n-1] == 0 and list1[i][n+1] == 0:
            Ham[i][i] +=0.25 * j2
        elif list1[i][n-1] == 1 and list1[i][n+1] == 0:
            Ham[i][i] += -0.25 * j2
        elif list1[i][n-1] == 0 and list1[i][n+1] == 1:
            Ham[i][i] += -0.25 * j2
        else:
            Ham[i][i] += 0.25 * j2
